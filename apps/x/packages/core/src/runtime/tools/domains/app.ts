...
import { DhowAppManifestSchema } from "@x/shared/dist/dhow-app.js";
...
export const appNavigationTools: z.infer<typeof BuiltinToolsSchema> = {
    'app-navigation': {
        ...
        inputSchema: z.object({
            ...
            // open-app
            appId: z.string().optional().describe("App folder slug under ~/.dhow/apps (for open-app) — opens the app in the middle pane."),
            // open-view / read-view
        ...
        execute: async (input: {
            ...
            switch (input.action) {
                ...
                case 'open-app': {
                    ...
                    let appName = appId;
                    try {
                        const raw = await fs.readFile(path.join(WorkDir, 'apps', appId, 'dhow-app.json'), 'utf-8');
                ...
                case 'read-view': {
                    ...
                    try {
                        switch (view) {
                            ...
                            case 'apps': {
                                // Installed/local Dhow apps — the copilot uses this to
...
export const appDataTools: z.infer<typeof BuiltinToolsSchema> = {
    'app-read-data': {
        ...
        execute: async ({ appFolder, file }: { appFolder: string; file?: string }) => {
            try {
                ...
                try {
                    DhowAppManifestSchema.parse(JSON.parse(await fs.readFile(path.join(dir, 'dhow-app.json'), 'utf-8')));
                } catch {
                    return { success: false, error: `No app "${appFolder}" (missing or invalid dhow-app.json).` };
                }
    ...
    'app-set-data': {
        permission: "none",
        description: "Write a Dhow App's data file — JSON its frontend reads via GET /_dhow/data/<file>. Deterministic: you supply the content, code handles the path, atomicity (temp→rename), and the app's dataContracts validation. This is how a background task refreshes an app's data — the agent RETURNS the data; never hand-write files under apps/.",
        inputSchema: z.object({
            appFolder: z.string().describe('The app folder slug under ~/.dhow/apps.'),
        ...
        execute: async ({ appFolder, file, data }: { appFolder: string; file: string; data: unknown }) => {
            try {
                ...
                const dir = path.join(WorkDir, 'apps', appFolder);
                let manifest: z.infer<typeof DhowAppManifestSchema>;
                try {
                    manifest = DhowAppManifestSchema.parse(JSON.parse(await fs.readFile(path.join(dir, 'dhow-app.json'), 'utf-8')));
                } catch {
                    return { success: false, error: `No app "${appFolder}" (missing or invalid dhow-app.json).` };
                }
