...
\`\`\`iframe
{"url": "http://example-dashboard.apps.localhost:3210/?__dhow_embed=1", "title": "Trend Dashboard", "height": 640}
\`\`\`
- \`url\` (required): Full URL to render. Use \`https://\` for remote sites, or a Dhow App origin (\`http://<folder>.apps.localhost:3210/?__dhow_embed=1\`) for local dashboards
...
- Remote sites may refuse to render in iframes because of their own CSP / X-Frame-Options headers. When you need a reliable embed, build a Dhow App (see the apps skill) and embed its origin with \`?__dhow_embed=1\`
...
- For local dashboards and mini apps, build a Dhow App (apps skill) and point an \`iframe\` block at \`http://<folder>.apps.localhost:3210/?__dhow_embed=1\`
