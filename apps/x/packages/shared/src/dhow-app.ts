...
// Dhow Apps schemas (spec §4.2, §5.1, §9.1, §11.4, §12.2).

export const PACKAGE_NAME_RE = /^[a-z0-9]+(-[a-z0-9]+)*$/;
export const SEMVER_RE = /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/;

// ---------------------------------------------------------------------------
// Manifest — dhow-app.json (§4.2)
// ---------------------------------------------------------------------------

export const DhowAppManifestSchema = z.object({
    ...
    capabilities: z.array(z.string()).default([])
        .describe('Capability identifiers this app may use (D7): Composio toolkit slugs for /_dhow/tools/*, plus the reserved identifiers "llm" (§7.6) and "copilot" (§7.7). Empty = none.'),
    ...
    build: z.object({ command: z.string() }).optional()
        .describe('RESERVED. Dhow MUST NOT execute this in V1.'),
    minDhowVersion: z.string().regex(SEMVER_RE).optional()
        .describe('RESERVED. Minimum compatible Dhow version; not enforced in V1.'),
}).passthrough(); // unknown fields survive round-trips (forward compatibility)

export type DhowAppManifest = z.infer<typeof DhowAppManifestSchema>;

// ---------------------------------------------------------------------------
// Install record — .dhow-install.json (§12.2)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Publish record — .dhow-publish.json (§11.4)
// ---------------------------------------------------------------------------
...
export const AppSummarySchema = z.object({
    ...
    manifest: DhowAppManifestSchema.optional(), // present when ok
