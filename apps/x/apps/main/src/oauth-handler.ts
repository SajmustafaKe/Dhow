...
export async function connectProvider(provider: string, credentials?: { clientId: string; clientSecret?: string }): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    // The Dhow account has no build-time client id under DCR — availability
    // depends entirely on whether the hosted app's bootstrap config
    // resolves a supabaseUrl, since that is what supplies the issuer to
    // discover against (auth/providers.ts). Probe it explicitly, with its
    // own message, rather than letting a raw fetch/parse error surface as
    // "Unknown error" from deep inside getProviderConfig below.
    if (provider === 'dhow') {
      try {
        await getProviderConfig('dhow');
      } catch (error) {
        console.error('[OAuth] Dhow bootstrap config unavailable:', error);
        return {
          success: false,
          error: 'Sign in with Dhow is not available right now. Check your connection and try again.',
        };
      }
    }
