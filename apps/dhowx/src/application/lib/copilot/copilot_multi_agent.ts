...
function findUsingDhowDocsDir(): string | null {
  const candidates = [
    path.resolve(process.cwd(), '../docs/docs/using-dhow'),
    path.resolve(process.cwd(), 'apps/docs/docs/using-dhow'),
  ];
...
function collectDocsSummaries(): string {
  const dir = findUsingDhowDocsDir();
  if (!dir) return '';
...
const USING_DHOW_DOCS = collectDocsSummaries();

// Inject auto-loaded docs, if available
export const COPILOT_INSTRUCTIONS_MULTI_AGENT_WITH_DOCS =
  COPILOT_INSTRUCTIONS_MULTI_AGENT.replace('{USING_DHOW_DOCS}', USING_DHOW_DOCS);
