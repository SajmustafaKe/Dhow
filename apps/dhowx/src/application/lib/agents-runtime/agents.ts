...
export const HandoffContext = z.object({
    ...
    metadata: z.union([z.record(z.string(), z.any()), z.string(), z.null()]).default(null)
});
...
export const PipelineContext = HandoffContext.extend({
    ...
    pipelineData: z.union([z.record(z.string(), z.any()), z.string(), z.null()]).default(null),
    stepResults: z.union([z.array(z.record(z.string(), z.any())), z.string(), z.null()]).default(null)
});
...
export const TaskContext = HandoffContext.extend({
    ...
    resources: z.union([z.record(z.string(), z.any()), z.string(), z.null()]).default(null)
});
...
export const PipelineExecutionState = z.object({
    ...
    pipelineData: z.union([z.record(z.string(), z.any()), z.string(), z.null()]).default(null),
    stepResults: z.union([z.array(z.record(z.string(), z.any())), z.string(), z.null()]).default(null),
    currentStepResult: z.union([z.record(z.string(), z.any()), z.string(), z.null()]).default(null),
...
`;
    ...
    const agent = new Agent({
        ...
        tools: agentTools,
        model: aisdk(openai.chat(config.model))
    });
            ...
            switch (event.type) {
                ...
                case 'run_item_stream_event':
                    ...
                    if (event.item.type === 'tool_call_output_item' &&
                        event.item.rawItem.type === 'function_call_result' &&
                        event.item.rawItem.status === 'completed' &&
                        typeof event.item.rawItem.output === 'object' &&
                        event.item.rawItem.output !== null &&
                        !Array.isArray(event.item.rawItem.output) &&
