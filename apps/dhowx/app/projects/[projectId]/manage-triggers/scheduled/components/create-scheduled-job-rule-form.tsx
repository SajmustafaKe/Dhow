...
import { ZonedDateTime, now, getLocalTimeZone, parseAbsoluteToLocal, parseDateTime, toZoned } from "@internationalized/date";
import { z } from "zod";
...
// `@heroui/react`'s DatePicker (with granularity="minute") is replaced by a native
// `<input type="datetime-local">`; these two converters bridge its "YYYY-MM-DDTHH:mm"
// string value and the ZonedDateTime the rest of the form operates on.
const zonedDateTimeToInputValue = (value: ZonedDateTime): string => {
    const pad = (n: number) => String(n).padStart(2, "0");
    return `${value.year}-${pad(value.month)}-${pad(value.day)}T${pad(value.hour)}:${pad(value.minute)}`;
};

const inputValueToZonedDateTime = (value: string, timeZone: string): ZonedDateTime => {
    return toZoned(parseDateTime(value), timeZone);
};
                ...
                <div className="max-w-[800px] mx-auto">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                type="datetime-local"
                                value={scheduledDateTime ? zonedDateTimeToInputValue(scheduledDateTime) : ""}
                                onChange={(e) => {
                                    if (!e.target.value) {
                                        setScheduledDateTime(null);
                                        return;
                                    }
                                    setScheduledDateTime(inputValueToZonedDateTime(e.target.value, placeholderDateTime.timeZone));
                                }}
                                min={zonedDateTimeToInputValue(minDateTime)}
                                required
