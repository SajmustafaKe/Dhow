...
export function ToolkitAuthModal({ 
  ...
  const handleComposioOAuth2 = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to connect to toolkit';
      setError(errorMessage);
    } finally {
      setProcessing(false);
    }
  ...
  const handleFormSubmit = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to authenticate with toolkit';
      setError(errorMessage);
    } finally {
      setProcessing(false);
    }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="bg-white dark:bg-gray-900">
                    ...
                    <div className="space-y-4">
                      {allFields.map(field => (
                        <div key={field.name} className="space-y-1.5">
                          <label
                            htmlFor={`toolkit-auth-field-${field.name}`}
                          ...
                          <Input
                            id={`toolkit-auth-field-${field.name}`}
                ...
                <div className="space-y-6">
                  ...
                  {(toolkit.composio_managed_auth_schemes.includes('OAUTH2') || 
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() => handleCustomAuth('OAUTH2')}
                      disabled={processing}
                      size="lg"
                    >
                  ...
                  {toolkit.auth_config_details?.filter(config => config.mode !== 'OAUTH2').map(config => (
                    <Button
                      ...
                      className="w-full justify-start gap-3 h-auto py-5 px-5"
                      variant="outline"
                      onClick={() => handleCustomAuth(config.mode)}
                      disabled={processing}
                      size="lg"
                    >
        ...
        <DialogFooter className="border-t border-gray-200 dark:border-gray-800 pt-4">
          {showForm ? (
            <>
              <Button variant="outline" onClick={handleBackToOptions} disabled={processing}>
                Back
              </Button>
              <Button 
