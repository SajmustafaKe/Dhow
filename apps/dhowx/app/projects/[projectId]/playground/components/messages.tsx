...
function MessageActionsMenu({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" aria-label="Message actions menu">
        {showExplain && (
          <DropdownMenuItem onSelect={onExplain} disabled={disabledExplain}>
            <HelpCircleIcon size={16} className="text-indigo-400 dark:text-indigo-300" />
            {explainLabel}
          </DropdownMenuItem>
        )}
        {showFix && (
          <DropdownMenuItem onSelect={onFix} disabled={disabledFix}>
            <FlagIcon size={16} className="text-orange-700 dark:text-orange-400" />
            {fixLabel}
          </DropdownMenuItem>
        )}
        {showJson && (
          <DropdownMenuItem onSelect={onJson} disabled={disabledJson}>
            <BracesIcon size={16} className="text-slate-500 dark:text-slate-300" />
            {jsonLabel}
          </DropdownMenuItem>
        )}
      </DropdownMenuContent>
