...
export function WorkflowEditor({
    ...
    const onRevertModalOpen = useCallback(() => setIsRevertModalOpen(true), []);
    const onRevertModalClose = useCallback(() => setIsRevertModalOpen(false), []);
    ...
    return (
        <EntitySelectionContext.Provider value={{
            ...
            <div className="h-full flex flex-col gap-5">
                            ...
                            <div className="absolute inset-0 z-20">
                                <div className="h-full overflow-auto">
                                    ...
                                    {state.present.selection?.type === "visualise" && (
                                        <Panel title={<div className="flex items-center justify-between w-full"><div className="text-base font-semibold text-gray-900 dark:text-gray-100">Agent Graph Visualizer</div><CustomButton variant="secondary" size="sm" onClick={handleHideVisualise} className="group"><span className="group-hover:hidden"><XIcon className="w-4 h-4" /></span><span className="hidden group-hover:inline">Close</span></CustomButton></div>}>
                ...
                <Dialog open={isRevertModalOpen} onOpenChange={(open) => { if (!open) onRevertModalClose(); }}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>
                        ...
                        <DialogFooter>
                            <CustomButton variant="ghost" className="text-red-600 hover:text-red-600" onClick={onRevertModalClose}>
                                Cancel
                            </CustomButton>
                            <CustomButton variant="destructive" onClick={handleConfirmRevert}>
                                Revert to Live
                            </CustomButton>
                        </DialogFooter>
                    </DialogContent>
                ...
                <Dialog
                    ...
                    onOpenChange={(open) => { if (!open) onPhoneModalClose(); }}
                >
                ...
                {/*
                <Dialog
                    open={isChatWidgetModalOpen}
                    onOpenChange={(open) => { if (!open) onChatWidgetModalClose(); }}
                >
