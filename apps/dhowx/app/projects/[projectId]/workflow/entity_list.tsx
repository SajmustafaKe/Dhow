...
// Small wrapper reproducing HeroUI Tooltip's `isDisabled` behavior: when
// disabled, render the trigger with no tooltip wrapper at all.
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                            {startAgentName !== pipeline.name ? (
                                <>
                                    <DropdownMenuItem onSelect={() => onSetMainAgent(pipeline.name)}>Set as start agent</DropdownMenuItem>
                                    <DropdownMenuItem variant="destructive" onSelect={() => onDeletePipeline(pipeline.name)}>Delete Pipeline</DropdownMenuItem>
                                </>
                            ) : (
                                <DropdownMenuItem variant="destructive" onSelect={() => onDeletePipeline(pipeline.name)}>Delete Pipeline</DropdownMenuItem>
                            )}
                            ...
                            <div className={`${headerClasses} rounded-md transition-colors h-full`}>
                                ...
                                <div className="flex items-center gap-1">
                                    {SHOW_VISUALIZATION && (
                                        ...
                                        >
                                            <span className="group-hover:hidden"><Eye className="w-4 h-4" /></span>
                                            <span className="hidden group-hover:inline">Visualise Agents</span>
                                        </Button>
                                    )}
                                     <Button
                                         variant="secondary"
                                         ...
                                         className={`group ${buttonClasses}`}
                                    >
                                        <span className="group-hover:hidden"><PlusIcon className="w-4 h-4" /></span>
                                        <span className="hidden group-hover:inline">Add Agent</span>
                                    </Button>
                                </div>
                            </div>
                        }
                        ...
                        title={
                            <div className={`${headerClasses} rounded-md transition-colors h-full`}>
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    >
                                        <span className="group-hover:hidden"><PlusIcon className="w-4 h-4" /></span>
                                        <span className="hidden group-hover:inline">Add Tool</span>
                                    </Button>
                                </div>
                            </div>
                        }
                                                                    ...
                                                                    <div className="mt-2">
                                                                        {customTools.map((tool, index) => (
                                                                            ...
                                                                            >
                                                                                ...
                                                                                {!tool.isLibrary && (
                                                                                    <InfoTooltip content="Remove tool">
                                                                                        <button
                        ...
                        title={
                            <div className={`${headerClasses} rounded-md transition-colors h-full`}>
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    >
                                        <span className="group-hover:hidden"><PlusIcon className="w-4 h-4" /></span>
                                        <span className="hidden group-hover:inline">Add Data Source</span>
                                    </Button>
                                </div>
                            </div>
                        }
                                    ...
                                    {dataSources.length > 0 ? (
                                        <div className="space-y-1">
                                            {dataSources.map((dataSource, index) => {
                                                ...
                                                if (isPending) {
                                                    statusPill = (
                                                        <InfoTooltip content="Processing">
                                                    ...
                                                    statusPill = (
                                                        <InfoTooltip content={dataSource.error || "Error"}>
                        ...
                        <Panel 
                            ...
                            title={
                                <div className={`${headerClasses} rounded-md transition-colors h-full`}>
                                    ...
                                    >
                                        <span className="group-hover:hidden"><PlusIcon className="w-4 h-4" /></span>
                                        <span className="hidden group-hover:inline">Add Variable</span>
                                    </Button>
                                </div>
                            }
                        >
...
function AgentDropdown({
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent>
                <DropdownMenuItem
                    disabled={agent.locked || isStartAgent}
                    onSelect={() => onSetMainAgent(agent.name)}
                >
                    Set as start agent
                </DropdownMenuItem>
                <DropdownMenuItem
                    disabled={!agent.toggleAble || isStartAgent}
                    onSelect={() => onToggle(agent.name)}
                >
...
const ComposioCard = ({
    ...
    if (!isToolkitConnected && hasToolkitWithAuth) {
        statusPill = (
            <InfoTooltip content="Toolkit needs to be connected">
                <button
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent>
                    ...
                    <DropdownMenuItem
                        ...
                        onSelect={() => { setShowRemoveToolkitModal(true); }}
                    >
                        {isProcessingRemove ? (
...
function AgentTypeModal({ isOpen, onClose, onConfirm, onCreatePipeline }: AgentTypeModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
            <DialogContent className="sm:max-w-5xl w-full p-0 gap-0">
