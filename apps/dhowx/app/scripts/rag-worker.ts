...
async function runProcessFilePipeline(_logger: PrefixLogger, usageTracker: UsageTracker, job: z.infer<typeof DataSource>, doc: z.infer<typeof DataSourceDoc>) {
    ...
    if (!USE_GEMINI_FILE_PARSING) {
        ...
        const { text, usage } = await generateText({
            model: openai.chat(FILE_PARSING_MODEL),
        ...
        markdown = text;
        // inputTokens/outputTokens are possibly undefined in ai@7 (the SDK no
        // longer guarantees the provider always reports them); usage is
        // billing-critical, so fall back to 0 rather than letting `undefined`
        // reach the tracker (which would otherwise turn into `NaN` credits),
        // and log loudly so a genuinely missing count doesn't go unnoticed.
